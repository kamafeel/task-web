...
var showDimInfoDialog=function(acttype){
		...
		dimCfg = ({
			...
			items : [ 
				{type:'combox',label:'所属组',notNull:'N',fieldName :'DIM_GROUP_ID',storesql:getComboxStoreSql('SELECT xmlid AS K ,group_value AS V FROM proc_schedule_dim_group order by group_seq','METADBS'),width:300},
...
$(document).ready(function() {
   ...
   $("#delDim").click(function(){
   		...
   		if(confirm("确定删除所有选中项吗?")){
   			...
   			ai.executeSQL("delete from proc_schedule_dim where xmlid in (" + xmlids + ")",false,"METADBS");
   			dimStore.select();
   			groupStore.select();
   			buildGroupList();
   			alert("已删除！");
