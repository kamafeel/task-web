...
var _grid = "";
var _editPanel="";
var _sql="SELECT xmlid,dataname,datacnname,dbname,level_val,topicname,cycletype,remark FROM tablefile WHERE 1=1 {condi}";
var _treeSql="select dbname, level_val, topicname, cycletype ,count(1) num from (" + _sql + ") tablefile group by dbname,level_val,topicname,cycletype";
...
function showDialog(actType){
	...
	_editPanel = new AI.Form({
		...
		items: [
			...
			{type : 'combox', label : '层次', notNull: 'N', fieldName : 'LEVEL_VAL', isReadOnly: isRead, width : 220, storesql:"SELECT  rowname  FROM metaedimdef WHERE dimcode='DIM_DATALEVEL'" },
			{type : 'combox', label : '主题', notNull: 'N', fieldName : 'TOPICNAME', isReadOnly: isRead, width : 220, storesql:"SELECT  rowname  FROM metaedimdef WHERE dimcode='DIM_TOPIC'" },
			{type : 'combox', label : '周期', notNull: 'N', fieldName : 'CYCLETYPE', isReadOnly: isRead, width : 220, storesql:"SELECT  rowname  FROM metaedimdef WHERE dimcode='DIM_TOPIC'"  },
...
var buildTreeView = function(sql){
	$('#treeview6').empty().treeview({
		...
		groupfield:"DBNAME,LEVEL_VAL,TOPICNAME,CYCLETYPE",
		sql:sql,
		dataSource:"METADB",
		subtype:'grouptree',
...
$(document).ready(function(){
	...
	_grid = new AI.Grid({
		...
		columns: [
			...
			{header: "层次", dataIndex: 'LEVEL_VAL', className: "ai-grid-body-td-center"},
			{header: "主题", dataIndex: 'TOPICNAME', className: "ai-grid-body-td-center"}
		]
	});
