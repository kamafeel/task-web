...
.show-level>span:hover {
	color: #91d3ea;
}
.show-level>span.active {
...
$(function(){
	...
	//调度监控必须传递seqno
	if(_seqno){
		var findSql = " select a.seqno,a.xmlid,a.agent_code,b.proc_name,a.run_freq,a.date_args,a.errcode,a.task_state,a.start_time,a.exec_time,a.end_time,a.use_time,b.proctype,'PROC' node_type "
		...
		switch(_op){
			...
			case "flow":
				...
				break;
			case "manual_log":
				var _proc_name=paramMap["proc_name"]||"";
				if(_proc_name.length<1){
					alert("缺少参数");
					return false;
				}
				//showManualLog(_seqno,_proc_name);
				readManualLog(_seqno,_proc_name);
				break;
			default:
				alert("未知的操作")
				break;
...
function showFlow(data){
	...
	$("#flowSwitchContainer").append('<div class="show-level">'
			+ '<button id="preDialog">前置</button>'
			+ '<button id="nextDialog">影响</button>'
			+ '<button id="allDialog">展示全部</button>'
			+ '</div>');
	bindFlowClick();//为流程图的按钮绑定事件
...
function showCondition(_seqno){
	var _procName = logStore.root[0].PROC_NAME;
	var errCode = "";
	$.ajax({
		url:'/'+contextPath+'/taskOpt/getWaitCode?seqno='+_seqno,
		async: false,
		error:function(){     
		       alert('请求服务' + this.url + '失败！');
		       return;
		    },
		success:function(msg){
			var msg = $.parseJSON(msg);
			if(msg.flag=='true'){
			     errCode = msg.response;
			}else{
				alert('获取排队等待原因失败');
				return false;
			}
    ...
    $("#panel1 #op-panelContent").append(
			...
			+ ((status==-7 || status==1)?'<span class="pull-right btn btn-danger btn-xs" id="setCondiSuccess">强制执行</span>':'')
			+ '</header> '
			+ '</section>'
	...
	$('#op-panelContent #setCondiSuccess').on('click',function(){
		if(confirm("确定要强制执行")){
	    	...
	    	$.ajax({
				...
				url: '/'+contextPath+'/taskOpt/forceExec',
				data: {
					"seqno": _seqno
				},
				async:false,
	...
	var config={
		...
		columns:[
			 ...
			 {"header":"检测通过","dataIndex":"CHECK_FLAG",
				 render:function(value){ 
					 return _.template('<span class="glyphicon glyphicon-<%=CHECK_FLAG==0?"remove":"ok"%>" style="color:#000000"></span>')
					 ({'CHECK_FLAG':value.get("CHECK_FLAG")});
				 }
			 }
		]
    };
...
function showLog(seqno){
	var _title = logStore.root[0].PROC_NAME;
	...
	var log='';
	$.ajax({
		url:'/'+contextPath+'/syn/getTaskLog/'+seqno,
		async: false,
		error:function(){     
		       alert('请求服务' + this.url + '失败！');
		       return;
		...
		success:function(msg){
			var msg = $.parseJSON(msg);
			if(msg.reCode==200 || msg.reCode=="200"){
			     log = msg.reInfo;
			}else{
				alert('获取运行日志失败:'+msg.reMsg);
				return false;
			}
		}
	});
	...
	if(log.length>0){
		...
		var time = logStore.root[0].STATUS_TIME;
		var retryNum = logStore.root[0].RETRYNUM?logStore.root[0].RETRYNUM:0;
...
function showRunningLog(seqno){
	...
	var _title = logStore.root[0].PROC_NAME;
	var _taskState = logStore.root[0].TASK_STATE;
	var _dataArgs = logStore.root[0].DATE_ARGS;
	var _run_freq = logStore.root[0].RUN_FREQ;
	_taskState = _taskState>=50?7:_taskState;
	_taskState = _taskState<0&&_taskState>=-3?8:_taskState;
	_taskState = _taskState==0?9:_taskState;
	_taskState = _taskState==-5?10:_taskState;
	...
	_dataArgs=_dataArgs.substring(0,10);
	
	var log = '';
	var tmpl = '';
	$.ajax({
		url:'/'+contextPath+'/syn/getLog?SEQNO='+seqno+'&AGENT_CODE='+agentCode+'&DATADIR='+_dataArgs+'&CMD=tail -n500 ',
		async:false,
		error:function(){     
		       alert('网络错误！');
		       return;
	...
	if(log.length>0){
		...
		var cla = _CLASS[_taskState-1];
		var name = _VALUE[_taskState-1];
		var time = logStore.root[0].UPDATE_TIME;
		var retryNum = logStore.root[0].RETRYNUM?0:logStore.root[0].RETRYNUM;
	...
	$el.on("finishRender",function(){					
			...
			if($el.find('#flot-bar-h').length==1){
				...
				if(dbType=="mysql"){
					_sql="select * from"
						+ " (select * from"
						+ " (select distinct date_args,timestampdiff(minute,exec_time,end_time) as duration from"
						+ " proc_schedule_log where xmlid='"+_xmlid+"') t"
						+ " where t.duration is not null order by t.date_args desc limit 15) t1 order by t1.date_args";
				}else{
					_sql="select * from"
						+ " (select * from"
						+ " (select distinct date_args,round(to_number(to_date(end_time,'yyyy-mm-dd hh24:mi:ss')-to_date(exec_time,'yyyy-mm-dd hh24:mi:ss'))*24*60) as duration from"
						+ " proc_schedule_log where xmlid='"+_xmlid+"') t"
						+ " where t.duration is not null and rownum<=15 order by t.date_args desc) t1 order by t1.date_args";
				}
				...
				for(i=0;i<_procTimerStore.getCount();i++){
					_categories.push(_procTimerStore.root[i].DATE_ARGS);
					_series.push(_procTimerStore.root[i].DURATION);
				}
				/*
				_procTimerStore.root.each(function(dt){
					_categories.push(dt.get("DATE_ARGS"));
...
//直接从数据库获取日志
function showManualLog(seqno,procName){
	var logStore = new AI.JsonStore({
        sql:"select seqno,proc_name,app_log from proc_schedule_script_log where run_freq='manual' and seqno='"+seqno+"'",
        key:"SEQNO",
        pageSize:-1,
        dataSource:"METADBS",
        table:"PROC_SCHEDULE_SCRIPT_LOG"
    });
    var r=  logStore.getRecordByKey(seqno);
    var _log =r==null?"无数据":r.get("APP_LOG"); 
	var _title = procName +"的日志";
	var tmpl = _.template(
			'<section class="panel panel-default">'
			+ '<header class="panel-heading"> 脚本运行日志</header> '
			+ '<article class="media">'
			...
			+ '</article>'
			+'</section>'
			)({"name":procName,"log":_log});
	openTableInfo("log",_title,tmpl,true);
}

//读取文件日志
function readManualLog(seqno,procName){
	var _log="";
	$.ajax({
		url:'/'+contextPath+'/syn/getTaskLog/'+seqno,
		async: false,
    ...
    _log = _log.length==0?"未找到日志文件":_log;
    
	var _title = procName +"运行日志";
	var tmpl = _.template(
			'<section class="panel panel-default">'
			+ '<header class="panel-heading"> 任务序列：<%=seqno%></header> '
			+ '<article class="media">'
			+ '<div class="media-body" style="margin:0px 40px 40px 40px;">'
			+ '<small class="block"><span>日志内容：</span></small>'
			+ '<small class="block" ><pre><%=log%></pre></small>'
			...
			+ '</article>'
			+'</section>'
			)({"seqno":seqno,"name":procName,"log":_log});
	openTableInfo("log",_title,tmpl,true);
}
...
<body>
	...
	<div id="flowSwitchContainer"></div>
