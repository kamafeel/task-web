...
var _sql= "select a.server_id,a.host_name,a.deploy_path,a.server_status,a.status_time,b.ipaddr,b.port,b.user_name from aietl_servernode a "+
		  ...
		  " where 1=1 {condi}";
var _treeSql=" select cast(server_status as char(2)) server_status, (case when server_status=1 then '正常' when server_status =0 then '待运行' else '异常' end) status,count(1) num from (" + _sql + ") agentList group by server_status ";
...
function StartAndStop(serverCode,status){
	var type= status=="0"?"启动":"停止";
	var str="确定要" + type + serverCode+"吗？";
	var op =status=="0"?"start":"stop";

	var url = '/' + contextPath + '/control/server/' + op + '?code=' + serverCode;
	if(confirm(str)){
		$.ajax({
			url:url,
			beforeSend:function(){
			...
			success:function(msg){
				if(msg.reCode == 200){
					msg = msg.reMsg + ",请等待zookeeper调整状态";
				}else {
					msg = msg.reMsg;
				}
				alert(msg)
