...
$(document).ready(function() {
	...
	function getsmsGroupName(v){
  	...
  	var warnTypeStore = new AI.JsonStore({
		sql:"select * from proc_schedule_dim where dim_group_id in (select xmlid from proc_schedule_dim_group) order by dim_seq",
		key:"XMLID",
	    pageSize:-1,
	    dataSource:"METADBS"
    });
	...
	var buildForm = function(){
		...
		var form = new AI.Form({							//表单组件
			...
			items : [ 										//表单内容
				...
				{type : 'checkbox',label : '告警类型',fieldName : 'ALARM_TYPE',notNull:'N',storesql: getComboxStoreSql("select dim_code as K,dim_value as V from proc_schedule_dim a,proc_schedule_dim_group b where a.dim_group_id = b.xmlid and b.group_code='ALARM_TYPE' order by dim_seq","METADBS")},
