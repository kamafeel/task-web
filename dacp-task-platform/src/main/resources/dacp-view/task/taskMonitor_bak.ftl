...
function showProcScheduleInfo(xmlid){
	...
	var formcfg = ({
		...
		items : [ 
			...
			{type : 'radio-custom',label : '运行模式',fieldName : 'MUTI_RUN_FLAG',storesql:'0,顺序启动|1,多重启动|2,唯一启动|3,周期内顺序启动',isReadOnly:"y"},
...
var _stateIcon = function(value,data,index){
	...
	if((dp_redo_type=='1' || dp_redo_type==1) && value>=50 && data.PROCTYPE == "taskTypeProc" ){ 
		li_redo_after =
			'<li class="dropdown-submenu">'+
				'<a tabindex="-1" href="javascript:void(0);">重做后续</a>'+
				'<ul class="dropdown-menu" >'+
					'<li> <a id="li_redo_after_0" name="li_redo_after_0" seq="<%=seqno%>" href="javascript:void(0);"><span class="glyphicon "></span>从头开始</a></li>'+
					'<li> <a id="li_redo_after_1" name="li_redo_after_1" seq="<%=seqno%>" returncode="<%=returnCode%>" href="javascript:void(0);"><span class="glyphicon "></span>从错误步骤开始</a></li>'+
					'<li class="" > <a id="li_redo_after_2" name="li_redo_after_2" dateArgs="<%=dateArgs%>"  procName="<%=procName%>" seq="<%=seqno%>"  returncode="<%=returnCode%>"  href="javascript:;">'+'<span class="glyphicon "></span>'+'临时重做</a></li>'+
				'</ul>'+
			'</li>';
		li_redo_self =
			'<li class="dropdown-submenu">'+
				...
				'<ul class="dropdown-menu" >'+
					'<li> <a id="li_redo_self_0" name="li_redo_self_0" seq="<%=seqno%>" href="javascript:void(0);"><span class="glyphicon "></span>从头开始</a></li>'+
					'<li> <a id="li_redo_self_1" name="li_redo_self_1" seq="<%=seqno%>"  returncode="<%=returnCode%>"  href="javascript:void(0);">'+'<span class="glyphicon "></span>从错误步骤开始</a></li>'+
				'</ul>'+
			'</li>';
	}
