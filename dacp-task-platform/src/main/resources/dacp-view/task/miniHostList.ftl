	...
	config:{
		...
		'events': {
			afterRender:function(){
				var _view = this;
				_view.$el.find("#query_title").empty().append('<ul class="nav navbar-nav"><li><a><i class="fa fa-home"> </i> 主机列表</a></li></ul>');
				_view.$el.find("#search").empty().append('<button type="button" class="btn btn-sm btn-default"><span class="glyphicon glyphicon-eye-open"></span>查询</button>');
				_view.$el.find("#add").empty().append('<button id="cre-ojb" type="button" class="btn btn-sm btn-default" style="margin:0px 5px"><span class="fa fa-plus"></span> 新增</button>')
				_view.$el.find("#delete").empty().append('<button id="delete-data" type="button" class="btn btn-sm btn-default"><span class="fa fa-trash-o"></span> 删除</button>')
			},		
...
function showDialog(actType){
	...
	if(_act == "add"){
		$("input[name='LOGIN_TYPE'][value='ssh']").click();
	}
