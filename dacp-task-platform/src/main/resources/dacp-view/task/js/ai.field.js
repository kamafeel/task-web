...
var  SelectBox = Class.$extend({
  ...
  init:function() {
   ...
   if(!this.config.sql) this.config.selectsql="select '' VALUES1,'' VALUES2,'' VALUES3 from (values(1))a";
	 ...
	 this.resultStore2 = new AI.JsonStore({
	  	   id:'resultStore2',
		   sql:this.config.resultsql,
		   dataSource:this.config.dataSource,
			pageSize:-1,
   ...
   $('#movetoleft').click(function(){
  	  var resultSet=resultGrid.getCheckedRows();
   	  ...
   	  for(var i=0;i<resultSet.length;i++){
   	  	var r=resultSet[i];
   	  	var r2 = self.resultStore2.getRecordByKey(r.get("VALUES1"))
		...
		if(r){
			self.resultStore.remove(r);
			self.resultStore2.remove(r2);
		}
   	  }
  ...
  $("#selectOK").click(function(){
  	//if(self.resultStore2.getCount()==0){alert('没有选择记录');return false;};
  	var result=[];
  	for(var i=0;i<self.resultStore2.getCount();i++){
  	  var r=self.resultStore2.getAt(i);
  	  result.push(r);
  	};
