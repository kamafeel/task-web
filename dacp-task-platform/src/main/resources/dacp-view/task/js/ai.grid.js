...
AI.Grid.prototype.buildBody = function() {
    ...
    if(this.store&&this.store.getCount()==0&&this.placeholder==true){
      ...
      $tbody.append('<tr style="padding:0" class="placeholder-img-tr"><td colspan="'+_colspan+'" style="padding:20px;text-align:center;width="'+_width+'" class="placeholder-img-td"><img id="placeholder-img" style="width:50%;" src="/'+_contextPath+'/dacp-view/aijs/images/table-placeholder-a.png" /></td></tr>');
    ...
    if(self.config.rightclick) {
        $("tr.ai-grid-body-tr",this.containerEl).off("mousedown").on("mousedown", function (e) {
            if (e.which == 3) {
                var rowIndex=$(this).attr("rowindex");
                var record = self.config.store.getAt($(this).attr("rowindex"));
                $("tr.active", this.containerEl).removeClass("selec");
