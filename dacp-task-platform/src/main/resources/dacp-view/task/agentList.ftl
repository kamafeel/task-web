...
var updateIPS = function(platform,toalIps){
	var totalStore = ai.getStore("select sum(ips_weight) TOTAL from aietl_agentnode where platform='"+platform+"' " , "METADBS");
	var total = (totalStore.root[0])["TOTAL"];
	if(total){
		var updateSql = "update aietl_agentnode set ips=(("+toalIps+"*ips_weight)/"+total+") where platform='"+platform+"'";
...
$(document).ready(function() {
    ...
    var platformSql = "SELECT a.platform platform,platform_cnname,ips,a.team_code,NUM FROM proc_schedule_platform a LEFT JOIN  (SELECT platform,COUNT(platform) NUM FROM aietl_agentnode GROUP BY platform) b ON a.platform=b.platform ORDER BY NUM DESC";
   	var agentSql = "select agent_name,task_type,node_ip,node_status,status_chgtime,a.ips,a.ips_weight,a.curips,script_path,platform_cnname,c.user_name, c.password, c.login_type, c.ipaddr, c.port, c.op_user, c.op_time, c.host_name, c.hostcnname from aietl_agentnode a left join proc_schedule_platform b on a.platform = b.platform left join dp_host_config c on a.host_name=c.host_name where  1=1 {}";
    ...
    var showPlatformInfoDialog = function(acttype){
		...
		var formcfg = ({
			...
			{type:'text-button',label:'所属团队',fieldName :'TEAM_CODE',notNull:'Y'}
			]
		});
	...
	var buildAgentList = function(){
		...
		agentGrid = new AI.Grid({
			...
			columns:[
				...
				{header: "资源权重", width:74, dataIndex: 'IPS_WEIGHT',render:ipsRender},
   ...
   var showAgentInfoDialog=function(acttype){
		...
		var formcfg = ({
			...
			items : [ 
				...
				{type:'combox',label:'所在主机',notNull:'N',fieldName :'HOST_NAME',width:300},
				{type:'text',label:'资源权重',notNull:'N',fieldName:'IPS_WEIGHT',width:300},
		...
		$("#HOST_NAME").empty().append(renderSelectHtml(host_name,'SELECT HOST_NAME AS K ,HOST_NAME AS V FROM dp_host_config','METADBS'))
		$('#agentConfig').modal({
			show : true,
			backdrop:false
		});
   ...
   $("#findAgent").click(function(){
   	   ...
   	   whereCase +=key.length>0?" and (agent_name like'%"+key+"%' or c.host_name like '%"+key+"%')":"";
   	   agentListStore.select(agentSql.replace("{}",whereCase));
   });
   //创建
   ...
   $("#delAgent").click(function(){
	...
	$("#platformConfig #dialog-ok").click(function() {
		...
		var sql="";
		if(isAdd){
		   var platStore = new AI.JsonStore({
   				sql: "select * from proc_schedule_platform where platform='"+platform+"' or platform_cnname='" + platform_name + "'",
   				dataSource: "METADBS"
   			});
   			
   			if(platStore.count>0){
   				alert("平台编号或组名已存在！");
   				return;
   			}	
	...
	$("#agentConfig #dialog-ok2").click(function() {
		...
		var curips = record.get("IPS_WEIGHT");
