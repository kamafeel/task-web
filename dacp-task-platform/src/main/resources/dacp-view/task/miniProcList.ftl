...
$(document).ready(function() {
    ...
    var procSql=" SELECT a.xmlid xmlid, a.xmlid proc_id,a.proc_name,a.proccnname,a.topicname,a.LEVEL_VAL,a.proctype,a.cycletype,a.path,a.creater,a.eff_date,a.curdutyer,a.state_date, " +
		    	...
		    	" WHERE a.state<>'DELETED' {condi}  order by state_date desc ";
	...
	var buildTreeView = function(){
        ...
        var $tree = $('#treeview').treeview({
 			...
 			onNodeSelected: function(event, data) {
 				...
 				var nodeCondi = " and b.task_group like '" + selectVal + "%'";
 				nodeCondi += getQueryCondition() + nodeCondi;
 				procStore.select(procSql.replace("{condi}",nodeCondi));
 			},
		...
		if(rowdata){
			...
			var r = procStore.curRecord;
		  	...
		  	if(r.get('PROCTYPE')=='WIDETAB'){
		  		...
		  		var type="edit";
		  		if(r.data.STATE=="NEW" || r.data.STATE=="新建"){
		  			type="add";
		  		}
		  		var url = "${mvcPath}/ftl/task/proschedInfoAdd?type="+type+"&dataSource=METADBS&xmlid="+r.get('PROC_ID')+"&team_code="+team_code;
		    	window.open(url);
    ...
    $("#insertBtn").click(function(){
    	...
    	var url = "${mvcPath}/ftl/task/proschedInfoAdd?type=add&dataSource=METADBS&xmlid="+ai.guid()+"&team_code="+team_code;
    	window.open(url);
   });
	$("#publishBtn").on("click",function(){
		...
		if(dataArray.length>0){
			for(var i=0;i<dataArray.length;i++){
				...
				var updateInfo  = "update proc_schedule_info set EXP_TIME='2050-12-31' where xmlid='"+xmlid+"'" ;
				var updateProc = "update proc set state='PUBLISHED',EFF_DATE='"+_date+"' where xmlid='"+xmlid+"'" ;
				var MultiSql = '[\"'+updateInfo+'\",\"'+updateProc+'\"]';
				...
				if(result.success&&(result.success==true||result.success=='true')){
					...
					sql="update metaobj set state = 'PUBLISHED',EFF_DATE='"+_date+"' where xmlid='"+xmlid+"'";
					ai.executeSQL(sql,false,"METADB");
				}else{
					msg2 += proc_name+"上线失败\n";
					//修改开发库proc和metaobj状态
					sql="update proc set state = 'CHANGE',EFF_DATE='"+_date+"' where xmlid='"+xmlid+"'";
					ai.executeSQL(sql,false,"METADB");
					sql="update metaobj set state = 'CHANGE',EFF_DATE='"+_date+"' where xmlid='"+xmlid+"'";
					ai.executeSQL(sql,false,"METADB");
				}
				//记录上线日志
				taskOpLog("'"+proc_name+"'","上线",updateProc,result.success);
			}
			alert(msg1+"\n"+msg2);
			procStore.select();
		}else{
	...
	var dataMigrate = function(xmlid){
		...
		var flag = false;
			...
			var deleteSql3 = "delete from proc_step where xmlid='"+xmlid+"'";
	...
	$("#invalidBtn").on("click",function(){
		...
		if(dataArray.length>0){
			for(var i=0;i<dataArray.length;i++){
				...
				updateProc = "update proc set state='PUBLISHED',EFF_DATE='"+_date+"' where xmlid='"+xmlid+"'" ;
				updateInfo  = "update proc_schedule_info set EXP_TIME='"+exp_time+"' where xmlid='"+xmlid+"'" ;
				var MultiSql = '[\"'+updateInfo+'\",\"'+updateProc+'\"]';
				var result = ai.executeMultiSql(MultiSql.toString(), null, "METADBS");
				if(result.success&&(result.success==true||result.success=='true')){
					//失效开发库proc和metaobj状态
					sql="update proc set state = 'INVALID',EFF_DATE='"+_date+"' where xmlid='"+xmlid+"'";
					ai.executeSQL(sql,false,"METADB");
					sql="update metaobj set state = 'INVALID',EFF_DATE='"+_date+"' where xmlid='"+xmlid+"'";
					ai.executeSQL(sql,false,"METADB");
				}
			}
			//记录下线日志
	...
	$("#deleteBtn").on("click",function(){
        ...
        if(_indexArray.length==0){
        	...
        	if(confirm("确定删除?")){
			    var infoSql = " update proc set state='DELETED' where xmlid='{}'";
			    var msg = "";
        		for(var i=_indexArray.length-1;i>=0;i--){
        			var r = procStore.getAt(_indexArray[i]);
        			//只删除失效任务
					if(r.get("STATE")=="INVALID"){
						realInfoSql = infoSql.replace("{}",r.get("XMLID"));
						ai.executeSQL(realInfoSql,false,"METADBS");
					}else{
						msg +="\n" + r.get("PROC_NAME");
					}		
        		}
...
<body class="">
			...
			<div class="container-fluid" style="padding-left: 0px">
				<div class="collapse navbar-collapse" style="padding-left: 0px">
					...
					<form class="navbar-form navbar-left" role="search">
						...
						<button id="search-key" type="button" class="btn btn-success btn-sm">查找</button>
						<div class="form-group">
					<button id="insertBtn" type="button" class="btn btn-primary btn-sm">增加</button>
					<button id="publishBtn" type="button" class="btn btn-info btn-sm">发布</button>
			        <button id="invalidBtn" type="button" class="btn btn-danger btn-sm">失效</button>
			        <button id="deleteBtn" type="button" class="btn btn-danger btn-sm">删除</button>
			        <button id="refreshBtn" type="button" class="btn btn-info btn-sm">
	...
	<div class="ui-layout-west row pre-scrollable">
		<div id="treeview"></div>
	</div>
