...
var _grid = "";
var tableStore = "";
var _sql="select a.platform,c.platform_cnname,a.agent_name,CASE WHEN a.curips is null then 0 else a.curips end as curips,a.ips,a.host_name,a.node_status,a.script_path,d.ipaddr,d.port,d.user_name " +
...
var _treeSql="select platform,platform_cnname,cast(node_status as char) node_status,(case when node_status=1 then '正常' else '异常' end) status,count(1) num from (" + _sql + ") agentList group by platform,platform_cnname,node_status";
...
function StartAndStop(agentCode,status){
	...
	var url = '/' + contextPath + '/control/agent/' + op + '?code=' + agentCode;
	if(confirm(str)){
		$.ajax({
			url:url,
			beforeSend:function(){
