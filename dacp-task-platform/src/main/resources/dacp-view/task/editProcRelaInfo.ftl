...
var runFreq='${procSchedInfo.run_freq!""}';//从proc表中获取
var delRow = -1;
var dataSource=paramMap['dataSource']||"METADB";
...
function attrReadOnlyRender(group,runFreq,selval) {
	...
	var _input_h= [['H','小时'],['D','天'],['N','无']];
	var _input_mi= [['MI','分钟'],['N','无']];
	var _input_w= [['W','周'],['N','无']];
	var _output_y= [['Y','年']];
	var _output_m= [['M','月']];
	var _output_d = [['D','天']];
	var _output_h = [['H','小时']];
	var _output_mi = [['MI','分钟']];
	var _output_w= [['W','周'],['N','无']];
	if(group=="1.输入表"){
		...
		else if(runFreq=="week")_ops = _input_w;
		else _ops = _ops;
	}
	else if(group=="2.输出表"){
		...
		else if(runFreq=="week")_ops = _output_w;
		else _ops = _ops;
	}
	...
	$.each(tableList, function (i, item){
		 ...
		 if(type=="DATA"){
			 row={
				 ...
				 ,tableName: '<input id="tableName" class="form-control myInput" value="'+item.dataname+'" readonly>'
				 ,tableCnName: '<input id="tableCnName" class="form-control myInput" value="'+item.datacnname+'" readonly>'
				 ,dbName: '<input id="dbName" class="form-control myInput" value="'+item.dbname+'" readonly>'
				 ,freq:   freq
				 ,run_freq_offset:'<div class="form-inline">前<input id="run_freq_offset" style="width:50px;height:30px;background-color: #fff;" class="form-control" value="'+para[1]+'">期</div>'
				 ,delbtn: getDelBtn(item.xmlid)
		        };
		 }else{
			 row={
					  xmlid:  '<input id="xmlid" class="form-control myInput" value="'+item.xmlid+'"  type="hidden" />'
					 ,proc_name: '<input id="proc_name" class="form-control myInput" value="'+item.proc_name+'" readonly>'
					 ,proccnname: '<input id="proccnname" class="form-control myInput" value="'+item.proccnname+'" readonly>'
					 ,freq:   freq
					 ,run_freq_offset:'<div class="form-inline">前<input id="run_freq_offset" style="width:50px;height:30px;background-color: #fff;" class="form-control" value="'+para[1]+'">期</div>'
					 ,delbtn: getDelBtn(item.xmlid)
			        };			 
...
$(function() {
    ...
    $table.empty().datagrid({
      columns:[[
          ...
          {title: "周期", field: "freq"},
          {title: "依赖偏移", field: "run_freq_offset"},
    ...
    $('#editDependTable').bind("click",
    	function(){
    	    ...
    	    var selBox = new SelectBox({sql: selSql,selectedValue: selectValue,callback: afterSelect,dataSource:dataSource});
    	    selBox.show();
    ...
    function getFreq(runFreq){
    	var res = runFreq.substr(0,1).toUpperCase() + "-0";
    	switch(runFreq){
		case "year":
			res = "Y-0";
			break;
    ...
    $('#saveRela').bind("click",
        ...
        model.save({}, {
            ...
            success: function (response) {
				...
				if(xmlid.length>0){
					//修改开发库proc和metaobj状态
					ai.executeSQL("update proc set state = 'CHANGE' where xmlid='"+xmlid+"'",false,dataSource);
					ai.executeSQL("update metaobj set state = 'CHANGE' where xmlid='"+xmlid+"'",false,dataSource);
				}
            },
  	...
  	//只读控制
	if (typeof window.parent.paramMap["type"]=="undefined"){
...
function InsertTableRows(tableId,xmlid,name,cnname,dbname,group,type){
	 ...
	 if(type=="DATA"){
	   row={  
		  ...
		  freq:   freq,
		  run_freq_offset:'<div class="form-inline">前<input id="run_freq_offset" style="width:50px;height:30px;background-color: #fff;" class="form-control" value="'+0+'">期</div>',
